...
function calculateNTime(realTime) {
    ...
    // 調整後のミリ秒 = リアルタイムのミリ秒 * 速度係数 (N時間の世界での経過ミリ秒)
    ...
    const h_n = Math.floor(secondsIntoN / 3600); // N時間の時
    const m_n = Math.floor((secondsIntoN % 3600) / 60); // N時間の分
    const s_n = Math.floor(secondsIntoN % 60); // N時間の秒
    
    // ★ 改善点1: N時間を24時間に換算して表示する
    // N時間の 'h' を現実時間 'h' に換算する係数 = N / 24
    const nTimeRate = currentN / 24; 

    // N時間の経過秒数を使って、24時間換算の総秒数を計算
...
function renderAlarmMode() {
    document.getElementById('content-area').innerHTML = `
        ...
        <div style="text-align:center; padding: 10px 0;">
            <button id="add-alarm-btn" onclick="addAlarm()" class="add-button">
